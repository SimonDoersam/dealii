...
FE_RaviartThomas<dim>::initialize_quad_dof_index_permutation_and_sign_change()
{
  ...
  // Example: if the switches are (true | true | true) that means we flip the
      ...
      for (unsigned int case_no = 0; case_no < 8; ++case_no)
        {
          ...
          const bool face_orientation = (((case_no / 4) % 2) == 1);
          const bool face_flip        = (((case_no / 2) % 2) == 1);
          const bool face_rotation    = ((case_no % 2) == 1);
          ...
          unsigned int new_local_face_dof =
            local_face_dof +
            this->adjust_quad_dof_index_for_face_orientation_table[face_no](
          ...
          /*
           * Now compute if a sign change is necessary.
           */
          ...
          /*
           * Now flip all signs if the face is not in standard orientation.
           */
          if (!face_orientation)
            this->adjust_quad_dof_sign_for_face_orientation_table[face_no](
              local_face_dof, case_no) =
