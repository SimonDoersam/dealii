  ...
  if (dim == 3)
    {
      ...
      for (unsigned int f = 0; f < this->n_unique_quads(); ++f)
        {
          adjust_quad_dof_sign_for_face_orientation_table[f] = Table<2, bool>(
              ...
              spacedim>::initialize_quad_dof_index_permutation_and_sign_change()
{
  AssertThrow(false,
              ExcMessage(
                ...
                "form FE_PolyTensor that is final. This method is"
                "specific to the (vector) finite element used and "
                "determines how face_dofs are "
